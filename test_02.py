...
def render_styled_chart(header_text, chart_obj):
    ...
    # 🚨 CRITICAL COLOR FIX:
    # 1. Use theme="streamlit" inside st.altair_chart to ensure colors are applied.
    # 2. Keep interactive(False) to prevent zooming/panning.
    st.altair_chart(final_chart.interactive(False), use_container_width=True, theme="streamlit")
...
def load_data():
    try:
        ...
        if not os.path.exists("loandisbursereport-sep_2025.xlsx"):
...
try:
    # Use the uploaded image file (now confirmed to be accessible)
